...
//     assert(input->get_weight() == in_channels);
...
//     Matrix<T> *output = new_unified_matrix<T>(input->get_height(), out_channels);

//     dim3 block_size(32, 32);
//     dim3 grid_size((output->get_width() + block_size.x - 1) / block_size.x,
//         (output->get_height() + block_size.y - 1) / block_size.y);
//     con1d_1kernel<<<grid_size, block_size>>>(input, output, weights, biases);
//     return output;
// }
// template <typename T>
// Matrix<T> *STN3d(Matrix<T> *input, std::map<std::string, std::vector<T>> params) {
//     Matrix<T> *conv1 = Conv1d<T>(input, params["conv1_weight"], params["conv1_bias"], 3, 64);
//     Matrix<T> *conv2 = Conv1d<T>(conv1, params["conv2_weight"], params["conv2_bias"], 64, 128);
//     free_matrix(conv1);
//     Matrix<T> *conv3 = Conv1d<T>(conv2, params["conv3_weight"], params["conv3_bias"], 128, 1024);
//     free_matrix(conv2);
